...
hardware_interface::return_type LittlebotHardware::configure(
  ...
  RCLCPP_INFO(rclcpp::get_logger("LittlebotHardware"),
    "Port: %s", serial_port_.c_str());
  ...
  for (const hardware_interface::ComponentInfo & joint : info_.joints) {
    ...
    if (joint.state_interfaces[0].name != hardware_interface::HW_IF_POSITION) {
      RCLCPP_FATAL(
        rclcpp::get_logger("LittlebotHardware"),
        "Joint '%s' have '%s' first state interface. '%s' and '%s' expected.",
...
std::vector<hardware_interface::StateInterface>
  LittlebotHardware::export_state_interfaces() {
...
std::vector<hardware_interface::CommandInterface>
  LittlebotHardware::export_command_interfaces() {
  std::vector<hardware_interface::CommandInterface> command_interfaces;

  command_interfaces.emplace_back(hardware_interface::CommandInterface(
    left_wheel_name_,
    hardware_interface::HW_IF_VELOCITY,
    &left_command_velocity_));

  command_interfaces.emplace_back(hardware_interface::CommandInterface(
    right_wheel_name_,
    hardware_interface::HW_IF_VELOCITY,
    &right_command_velocity_));
...
hardware_interface::return_type LittlebotHardware::read() {
  ...
  std::string med_msg;
  do {
    serial_device_.ReceiveMsg(&message_protocol_);
